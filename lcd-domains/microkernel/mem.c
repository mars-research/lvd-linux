...
static int isolated_map_contiguous_mem_cpu(struct lcd *lcd,
				struct lcd_memory_object *mo,
				gpa_t base, int cpu)
{
	int ret;
...
static int isolated_map_memory_object_cpu(struct lcd *lcd, 
				struct lcd_memory_object *mo,
				gpa_t base, int cpu)
{
	/*
	 ...
	 */
	if (__lcd_memory_object_is_contiguous(mo))
		return isolated_map_contiguous_mem_cpu(lcd, mo, base, cpu);
	else
		return printk("%s, not implemented for vmalloc mem\n", __func__);
}
