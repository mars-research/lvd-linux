...
#define ISOLATED_MODULE_NAME	"acpi_power_meter"

cptr_t hwmon_klcd, pwmtr_lcd;
struct lcd_create_ctx *pwmtr_ctx;
...
static int boot_main(void)
{
	...
	m = lvd_create_module_klcd_no_thread(LCD_DIR("acpi_hwmon_vmfunc/hwmon_klcd"),
				"lcd_test_mod_acpi_hwmon_vmfunc_hwmon_klcd",
				&hwmon_klcd);
	...
	ret = lvd_create_module_lvd(LCD_DIR("acpi_hwmon_vmfunc/pwmtr_lcd"),
				"lcd_test_mod_acpi_hwmon_vmfunc_pwmtr_lcd",
				&pwmtr_lcd,
				&pwmtr_ctx, 1);
	if (ret) {
		LIBLCD_ERR("failed to create pwmtr lcd");
		goto fail4;
	}
	...
	ret = lcd_run(pwmtr_lcd);
	if (ret) {
		LIBLCD_ERR("failed to start pwmtr lcd");
		goto fail9;
	}
...
fail9:
fail8:
	lcd_cap_delete(pwmtr_lcd);
	lcd_destroy_create_ctx(pwmtr_ctx);
fail4:
	//lcd_cap_delete(hwmon_klcd);
	lcd_destroy_module_klcd(hwmon_klcd, "lcd_test_mod_acpi_hwmon_vmfunc_hwmon_klcd");
fail3:
