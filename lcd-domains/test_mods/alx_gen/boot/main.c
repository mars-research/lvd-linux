...
cptr_t net_klcd, alx_lcd;
struct lcd_create_ctx *alx_ctx;
...
static int boot_main(void)
{
	...
	m = lvd_create_module_klcd_no_thread(LCD_DIR("alx_gen/net_klcd"),
				"lcd_test_mod_alx_gen_net_klcd",
				&net_klcd);
	...
	ret = lvd_create_module_lvd(LCD_DIR("alx_gen/alx_lcd"),
				"lcd_test_mod_alx_gen_alx_lcd",
				&alx_lcd,
				&alx_ctx, 1);
	if (ret) {
...
fail9:
fail8:
	lcd_cap_delete(alx_lcd);
	lcd_destroy_create_ctx(alx_ctx);
fail4:
	//lcd_cap_delete(net_klcd);
	lcd_destroy_module_klcd(net_klcd, "lcd_test_mod_alx_gen_net_klcd");
fail3:
