...
int dm_klcd_dispatch_loop(struct fipc_message *msg)
{
...
int dm_klcd_syncipc_dispatch(struct fipc_message *message)
{
	glue_user_panic("Shouldn't have done a synchronous call!\n");
	return -1;
}

static int dm_klcd_init(void) 
{
	...
	vmfunc_init(lcd_stack, dm_klcd_dispatch_loop, dm_klcd_syncipc_dispatch);

	/* call module_init for lcd */
	m.vmfunc_id = VMFUNC_RPC_CALL;
	m.rpc_id = MODULE_INIT;
	LIBLCD_MSG("vmfunc_init successfull! Calling MODULE_INIT of dmzero_lcd");
 ...
 */
static void __exit dm_klcd_exit(void)
{
	struct fipc_message m = {0};
	...
	LIBLCD_MSG("Calling MODULE_EXIT of dmzero_lcd");
	vmfunc_klcd_wrapper(&m, OTHER_DOMAIN);
...
module_init(dm_klcd_init);
module_exit(dm_klcd_exit);
MODULE_LICENSE("GPL");
