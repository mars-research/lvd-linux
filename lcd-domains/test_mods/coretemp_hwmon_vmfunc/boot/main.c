...
#define ISOLATED_MODULE_NAME	"coretemp"
...
static int boot_main(void)
{
	...
	m = lvd_create_module_klcd_no_thread(LCD_DIR("coretemp_hwmon_vmfunc/hwmon_klcd"),
				"lcd_test_mod_coretemp_hwmon_vmfunc_hwmon_klcd",
				&hwmon_klcd);
	...
	ret = lvd_create_module_lvd(LCD_DIR("coretemp_hwmon_vmfunc/coretemp_lcd"),
				"lcd_test_mod_coretemp_hwmon_vmfunc_coretemp_lcd",
				&coretemp_lcd,
				&coretemp_ctx, 1);
	if (ret) {
...
fail8:
	...
	lvd_destroy_create_ctx(coretemp_ctx);
fail4:
	//lcd_cap_delete(hwmon_klcd);
	lcd_destroy_module_klcd(hwmon_klcd, "lcd_test_mod_coretemp_hwmon_vmfunc_hwmon_klcd");
fail3:
