...
static int boot_main(void)
{
	...
	m = lvd_create_module_klcd_no_thread(LCD_DIR("ixgbe_gen/net_klcd"),
				"lcd_test_mod_ixgbe_gen_net_klcd",
				&net_klcd);
	...
	ret = lvd_create_module_lvd(LCD_DIR("ixgbe_gen/ixgbe_lcd"),
				"lcd_test_mod_ixgbe_gen_ixgbe_lcd",
				&ixgbe_lcd,
				&ixgbe_ctx, 1);
	if (ret) {
...
fail8:
	...
	lvd_destroy_create_ctx(ixgbe_ctx);
fail4:
	//lcd_cap_delete(net_klcd);
	lcd_destroy_module_klcd(net_klcd, "lcd_test_mod_ixgbe_gen_net_klcd");
fail3:
