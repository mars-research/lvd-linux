...
void ndo_set_rx_mode_callee(struct fipc_message* msg, struct ext_registers* ext)
{
	...
	fptr_ndo_set_rx_mode function_ptr = glue_unpack(__pos, msg, ext, fptr_ndo_set_rx_mode);
...

	{
		*netdev_ptr = glue_unpack_shadow(__pos, msg, ext, struct net_device*);
		if (*netdev_ptr) {
			callee_unmarshal_kernel__ndo_set_rx_mode__netdev__in(__pos, msg, ext, ctx, *netdev_ptr);
		}
	...
	*__pos = 0;
	{
		if (*netdev_ptr) {
			callee_marshal_kernel__ndo_set_rx_mode__netdev__in(__pos, msg, ext, ctx, *netdev_ptr);
		}
...
void ndo_stop_callee(struct fipc_message* msg, struct ext_registers* ext)
{
	size_t n_pos = 0;
	size_t* __pos = &n_pos;

	fptr_ndo_stop function_ptr = glue_unpack(__pos, msg, ext, fptr_ndo_stop);
	struct net_device* netdev = 0;
	struct net_device** netdev_ptr = &netdev;
	int ret = 0;
	int* ret_ptr = &ret;
...

	{
		*netdev_ptr = glue_unpack_shadow(__pos, msg, ext, struct net_device*);
		if (*netdev_ptr) {
			callee_unmarshal_kernel__ndo_stop__netdev__in(__pos, msg, ext, ctx, *netdev_ptr);
		}
	...
	*__pos = 0;
	{
		if (*netdev_ptr) {
			callee_marshal_kernel__ndo_stop__netdev__in(__pos, msg, ext, ctx, *netdev_ptr);
		}
...
void ndo_open_callee(struct fipc_message* msg, struct ext_registers* ext)
{
	size_t n_pos = 0;
	size_t* __pos = &n_pos;

	fptr_ndo_open function_ptr = glue_unpack(__pos, msg, ext, fptr_ndo_open);
	struct net_device* netdev = 0;
	struct net_device** netdev_ptr = &netdev;
	int ret = 0;
	int* ret_ptr = &ret;
...

	{
		*netdev_ptr = glue_unpack_shadow(__pos, msg, ext, struct net_device*);
		if (*netdev_ptr) {
			callee_unmarshal_kernel__ndo_open__netdev__in(__pos, msg, ext, ctx, *netdev_ptr);
		}
	...
	*__pos = 0;
	{
		if (*netdev_ptr) {
			callee_marshal_kernel__ndo_open__netdev__in(__pos, msg, ext, ctx, *netdev_ptr);
		}
...
void ndo_start_xmit_callee(struct fipc_message* msg, struct ext_registers* ext)
{
	...
	fptr_ndo_start_xmit function_ptr = glue_unpack(__pos, msg, ext, fptr_ndo_start_xmit);
...

	{
		*skb_ptr = glue_unpack_shadow(__pos, msg, ext, struct sk_buff*);
		if (*skb_ptr) {
			callee_unmarshal_kernel__ndo_start_xmit__skb__io(__pos, msg, ext, ctx, *skb_ptr);
		}
...

	{
		*netdev_ptr = glue_unpack(__pos, msg, ext, struct net_device*);
		if (*netdev_ptr) {
			callee_unmarshal_kernel__ndo_start_xmit__netdev__in(__pos, msg, ext, ctx, *netdev_ptr);
		}
	...
	*__pos = 0;
	{
		...
		if (*skb_ptr) {
			callee_marshal_kernel__ndo_start_xmit__skb__io(__pos, msg, ext, ctx, *skb_ptr);
		}
	...
	{
		if (*netdev_ptr) {
			callee_marshal_kernel__ndo_start_xmit__netdev__in(__pos, msg, ext, ctx, *netdev_ptr);
		}
...
void ndo_features_check_callee(struct fipc_message* msg, struct ext_registers* ext)
{
	size_t n_pos = 0;
	size_t* __pos = &n_pos;

	fptr_ndo_features_check function_ptr = glue_unpack(__pos, msg, ext, fptr_ndo_features_check);
	struct sk_buff* skb = 0;
...

	{
		*skb_ptr = glue_unpack(__pos, msg, ext, struct sk_buff*);
		if (*skb_ptr) {
			callee_unmarshal_kernel__ndo_features_check__skb__in(__pos, msg, ext, ctx, *skb_ptr);
		}
...

	{
		*dev_ptr = glue_unpack_shadow(__pos, msg, ext, struct net_device*);
		if (*dev_ptr) {
			callee_unmarshal_kernel__ndo_features_check__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
	...
	*__pos = 0;
	{
		if (*skb_ptr) {
			callee_marshal_kernel__ndo_features_check__skb__in(__pos, msg, ext, ctx, *skb_ptr);
		}
	...
	{
		if (*dev_ptr) {
			callee_marshal_kernel__ndo_features_check__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
...
void ndo_set_mac_address_callee(struct fipc_message* msg, struct ext_registers* ext)
{
	size_t n_pos = 0;
	size_t* __pos = &n_pos;

	fptr_ndo_set_mac_address function_ptr = glue_unpack(__pos, msg, ext, fptr_ndo_set_mac_address);
	struct net_device* netdev = 0;
	void* p = 0;
	struct net_device** netdev_ptr = &netdev;
	void** p_ptr = &p;
...

	{
		*netdev_ptr = glue_unpack_shadow(__pos, msg, ext, struct net_device*);
		if (*netdev_ptr) {
			callee_unmarshal_kernel__ndo_set_mac_address__netdev__in(__pos, msg, ext, ctx, *netdev_ptr);
		}

	}

	{
		*p_ptr = glue_unpack(__pos, msg, ext, void*);
		if (*p_ptr) {
		}
	...
	*__pos = 0;
	{
		if (*netdev_ptr) {
			callee_marshal_kernel__ndo_set_mac_address__netdev__in(__pos, msg, ext, ctx, *netdev_ptr);
		}
...
void ndo_do_ioctl_callee(struct fipc_message* msg, struct ext_registers* ext)
{
	size_t n_pos = 0;
	size_t* __pos = &n_pos;

	fptr_ndo_do_ioctl function_ptr = glue_unpack(__pos, msg, ext, fptr_ndo_do_ioctl);
	struct net_device* netdev = 0;
	struct ifreq* req = 0;
	int cmd = 0;
	struct net_device** netdev_ptr = &netdev;
...

	{
		*netdev_ptr = glue_unpack_shadow(__pos, msg, ext, struct net_device*);
		if (*netdev_ptr) {
			callee_unmarshal_kernel__ndo_do_ioctl__netdev__in(__pos, msg, ext, ctx, *netdev_ptr);
		}

	}

	{
		*req_ptr = glue_unpack_new_shadow(__pos, msg, ext, struct ifreq*, (sizeof(struct ifreq)), (DEFAULT_GFP_FLAGS));
		if (*req_ptr) {
			callee_unmarshal_kernel__ndo_do_ioctl__req__in(__pos, msg, ext, ctx, *req_ptr);
		}
	...
	*__pos = 0;
	{
		if (*netdev_ptr) {
			callee_marshal_kernel__ndo_do_ioctl__netdev__in(__pos, msg, ext, ctx, *netdev_ptr);
		}
	...
	{
		if (*req_ptr) {
			callee_marshal_kernel__ndo_do_ioctl__req__in(__pos, msg, ext, ctx, *req_ptr);
		}
...
void ndo_change_mtu_callee(struct fipc_message* msg, struct ext_registers* ext)
{
	size_t n_pos = 0;
	size_t* __pos = &n_pos;

	fptr_ndo_change_mtu function_ptr = glue_unpack(__pos, msg, ext, fptr_ndo_change_mtu);
	struct net_device* netdev = 0;
	int new_mtu = 0;
	struct net_device** netdev_ptr = &netdev;
...

	{
		*netdev_ptr = glue_unpack(__pos, msg, ext, struct net_device*);
		if (*netdev_ptr) {
			callee_unmarshal_kernel__ndo_change_mtu__netdev__in(__pos, msg, ext, ctx, *netdev_ptr);
		}

	}

	{
		*new_mtu_ptr = glue_unpack(__pos, msg, ext, int);
	}
	...
	*__pos = 0;
	{
		if (*netdev_ptr) {
			callee_marshal_kernel__ndo_change_mtu__netdev__in(__pos, msg, ext, ctx, *netdev_ptr);
		}
...
void ndo_tx_timeout_callee(struct fipc_message* msg, struct ext_registers* ext)
{
	size_t n_pos = 0;
	size_t* __pos = &n_pos;

	fptr_ndo_tx_timeout function_ptr = glue_unpack(__pos, msg, ext, fptr_ndo_tx_timeout);
	struct net_device* netdev = 0;
	struct net_device** netdev_ptr = &netdev;
...

	{
		*netdev_ptr = glue_unpack(__pos, msg, ext, struct net_device*);
		if (*netdev_ptr) {
			callee_unmarshal_kernel__ndo_tx_timeout__netdev__in(__pos, msg, ext, ctx, *netdev_ptr);
		}
	...
	*__pos = 0;
	{
		if (*netdev_ptr) {
			callee_marshal_kernel__ndo_tx_timeout__netdev__in(__pos, msg, ext, ctx, *netdev_ptr);
		}
...
void ndo_get_stats64_callee(struct fipc_message* msg, struct ext_registers* ext)
{
	...
	fptr_ndo_get_stats64 function_ptr = glue_unpack(__pos, msg, ext, fptr_ndo_get_stats64);
	struct net_device* netdev = 0;
...

	{
		*netdev_ptr = glue_unpack(__pos, msg, ext, struct net_device*);
		if (*netdev_ptr) {
			callee_unmarshal_kernel__ndo_get_stats64__netdev__in(__pos, msg, ext, ctx, *netdev_ptr);
		}

	}

	{
		*stats_ptr = glue_unpack(__pos, msg, ext, struct rtnl_link_stats64*);
		if (*stats_ptr) {
			callee_unmarshal_kernel__ndo_get_stats64__stats__in(__pos, msg, ext, ctx, *stats_ptr);
		}
	...
	*__pos = 0;
	{
		if (*netdev_ptr) {
			callee_marshal_kernel__ndo_get_stats64__netdev__in(__pos, msg, ext, ctx, *netdev_ptr);
		}
	...
	{
		if (*stats_ptr) {
			callee_marshal_kernel__ndo_get_stats64__stats__in(__pos, msg, ext, ctx, *stats_ptr);
		}
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*ret_ptr) {
			callee_marshal_kernel__ndo_get_stats64__ret_rtnl_link_stats64__out(__pos, msg, ext, ctx, *ret_ptr);
		}
...
void ndo_setup_tc_callee(struct fipc_message* msg, struct ext_registers* ext)
{
	size_t n_pos = 0;
	size_t* __pos = &n_pos;

	fptr_ndo_setup_tc function_ptr = glue_unpack(__pos, msg, ext, fptr_ndo_setup_tc);
	struct net_device* dev = 0;
...

	{
		*dev_ptr = glue_unpack_shadow(__pos, msg, ext, struct net_device*);
		if (*dev_ptr) {
			callee_unmarshal_kernel__ndo_setup_tc__dev__io(__pos, msg, ext, ctx, *dev_ptr);
		}

	}

	{
		*handle_ptr = glue_unpack(__pos, msg, ext, unsigned int);
	}

	{
		*proto_ptr = glue_unpack(__pos, msg, ext, unsigned short);
	}

	{
		*tc_ptr = glue_unpack(__pos, msg, ext, struct tc_to_netdev*);
		if (*tc_ptr) {
			callee_unmarshal_kernel__ndo_setup_tc__tc__in(__pos, msg, ext, ctx, *tc_ptr);
		}
	...
	*__pos = 0;
	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*dev_ptr) {
			callee_marshal_kernel__ndo_setup_tc__dev__io(__pos, msg, ext, ctx, *dev_ptr);
		}
	...
	{
		if (*tc_ptr) {
			callee_marshal_kernel__ndo_setup_tc__tc__in(__pos, msg, ext, ctx, *tc_ptr);
		}
...
void ndo_fix_features_callee(struct fipc_message* msg, struct ext_registers* ext)
{
	size_t n_pos = 0;
	size_t* __pos = &n_pos;

	fptr_ndo_fix_features function_ptr = glue_unpack(__pos, msg, ext, fptr_ndo_fix_features);
	struct net_device* netdev = 0;
	unsigned long long features = 0;
	struct net_device** netdev_ptr = &netdev;
	unsigned long long* features_ptr = &features;
...

	{
		*netdev_ptr = glue_unpack(__pos, msg, ext, struct net_device*);
		if (*netdev_ptr) {
			callee_unmarshal_kernel__ndo_fix_features__netdev__in(__pos, msg, ext, ctx, *netdev_ptr);
		}
	...
	*__pos = 0;
	{
		if (*netdev_ptr) {
			callee_marshal_kernel__ndo_fix_features__netdev__in(__pos, msg, ext, ctx, *netdev_ptr);
		}
...
void ndo_set_features_callee(struct fipc_message* msg, struct ext_registers* ext)
{
	size_t n_pos = 0;
	size_t* __pos = &n_pos;

	fptr_ndo_set_features function_ptr = glue_unpack(__pos, msg, ext, fptr_ndo_set_features);
	struct net_device* netdev = 0;
	unsigned long long features = 0;
	struct net_device** netdev_ptr = &netdev;
	unsigned long long* features_ptr = &features;
...

	{
		*netdev_ptr = glue_unpack(__pos, msg, ext, struct net_device*);
		if (*netdev_ptr) {
			callee_unmarshal_kernel__ndo_set_features__netdev__in(__pos, msg, ext, ctx, *netdev_ptr);
		}
	...
	*__pos = 0;
	{
		if (*netdev_ptr) {
			callee_marshal_kernel__ndo_set_features__netdev__in(__pos, msg, ext, ctx, *netdev_ptr);
		}
...
void ndo_set_tx_maxrate_callee(struct fipc_message* msg, struct ext_registers* ext)
{
	size_t n_pos = 0;
	size_t* __pos = &n_pos;

	fptr_ndo_set_tx_maxrate function_ptr = glue_unpack(__pos, msg, ext, fptr_ndo_set_tx_maxrate);
	struct net_device* netdev = 0;
	int queue_index = 0;
...

	{
		*netdev_ptr = glue_unpack_shadow(__pos, msg, ext, struct net_device*);
		if (*netdev_ptr) {
			callee_unmarshal_kernel__ndo_set_tx_maxrate__netdev__in(__pos, msg, ext, ctx, *netdev_ptr);
		}

	}

	{
		*queue_index_ptr = glue_unpack(__pos, msg, ext, int);
	}

	{
		*maxrate_ptr = glue_unpack(__pos, msg, ext, unsigned int);
	}
	...
	*__pos = 0;
	{
		if (*netdev_ptr) {
			callee_marshal_kernel__ndo_set_tx_maxrate__netdev__in(__pos, msg, ext, ctx, *netdev_ptr);
		}
...
int del_timer_sync(struct timer_list* timer)
{
	...
	{
		const void* __adjusted = *timer_ptr;
		glue_pack(__pos, msg, ext, __adjusted);
		if (*timer_ptr) {
			caller_marshal_kernel__del_timer_sync__timer__in(__pos, msg, ext, ctx, *timer_ptr);
		}

	}

	glue_call_server(__pos, msg, RPC_ID_del_timer_sync);

	*__pos = 0;
	{
		if (*timer_ptr) {
			caller_unmarshal_kernel__del_timer_sync__timer__in(__pos, msg, ext, ctx, *timer_ptr);
		}
...
void pci_disable_msix(struct pci_dev* dev)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*dev_ptr) {
			caller_marshal_kernel__pci_disable_msix__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}

	}

	glue_call_server(__pos, msg, RPC_ID_pci_disable_msix);

	*__pos = 0;
	{
		if (*dev_ptr) {
			caller_unmarshal_kernel__pci_disable_msix__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
...
int pcie_get_minimum_link(struct pci_dev* dev, unsigned int* speed, unsigned int* width)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*dev_ptr) {
			caller_marshal_kernel__pcie_get_minimum_link__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}

	}

	{
		const void* __adjusted = *speed_ptr;
		glue_pack(__pos, msg, ext, __adjusted);
		if (*speed_ptr) {
	...
	{
		const void* __adjusted = *width_ptr;
		glue_pack(__pos, msg, ext, __adjusted);
		if (*width_ptr) {
	...
	glue_call_server(__pos, msg, RPC_ID_pcie_get_minimum_link);

	*__pos = 0;
	{
		if (*dev_ptr) {
			caller_unmarshal_kernel__pcie_get_minimum_link__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
...
int pcie_capability_read_word(struct pci_dev* dev, int pos, unsigned short* val)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*dev_ptr) {
			caller_marshal_kernel__pcie_capability_read_word__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
	...
	glue_call_server(__pos, msg, RPC_ID_pcie_capability_read_word);

	*__pos = 0;
	{
		if (*dev_ptr) {
			caller_unmarshal_kernel__pcie_capability_read_word__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
...
void pci_restore_state(struct pci_dev* dev)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*dev_ptr) {
			caller_marshal_kernel__pci_restore_state__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}

	}

	glue_call_server(__pos, msg, RPC_ID_pci_restore_state);

	*__pos = 0;
	{
		if (*dev_ptr) {
			caller_unmarshal_kernel__pci_restore_state__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
...
int pci_set_power_state(struct pci_dev* dev, int state)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*dev_ptr) {
			caller_marshal_kernel__pci_set_power_state__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
	...
	glue_call_server(__pos, msg, RPC_ID_pci_set_power_state);

	*__pos = 0;
	{
		if (*dev_ptr) {
			caller_unmarshal_kernel__pci_set_power_state__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
...
int pci_save_state(struct pci_dev* dev)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*dev_ptr) {
			caller_marshal_kernel__pci_save_state__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}

	}

	glue_call_server(__pos, msg, RPC_ID_pci_save_state);

	*__pos = 0;
	{
		if (*dev_ptr) {
			caller_unmarshal_kernel__pci_save_state__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
...
int mod_timer(struct timer_list* timer, unsigned long expires)
{
...

	{
		...
		glue_pack(__pos, msg, ext, __adjusted);
		if (*timer_ptr) {
			caller_marshal_kernel__mod_timer__timer__in(__pos, msg, ext, ctx, *timer_ptr);
		}

	}

	{
		glue_pack(__pos, msg, ext, *expires_ptr);
	}

	glue_call_server(__pos, msg, RPC_ID_mod_timer);

	*__pos = 0;
	{
		if (*timer_ptr) {
			caller_unmarshal_kernel__mod_timer__timer__in(__pos, msg, ext, ctx, *timer_ptr);
		}
...
int pci_wake_from_d3(struct pci_dev* dev, bool enable)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*dev_ptr) {
			caller_marshal_kernel__pci_wake_from_d3__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}

	}

	{
		glue_pack(__pos, msg, ext, *enable_ptr);
	}

	glue_call_server(__pos, msg, RPC_ID_pci_wake_from_d3);

	*__pos = 0;
	{
		if (*dev_ptr) {
			caller_unmarshal_kernel__pci_wake_from_d3__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
...
void* ioremap_nocache(unsigned long long phys_addr, unsigned long size)
{
	...
	{
		glue_pack(__pos, msg, ext, *phys_addr_ptr);
	}
	...
	glue_call_server(__pos, msg, RPC_ID_ioremap_nocache);
...
unsigned long msleep_interruptible(unsigned int msecs)
{
	...
	{
		glue_pack(__pos, msg, ext, *msecs_ptr);
	}

	glue_call_server(__pos, msg, RPC_ID_msleep_interruptible);
...
void netdev_rss_key_fill(void* buffer, unsigned long len)
{
	...
	glue_call_server(__pos, msg, RPC_ID_netdev_rss_key_fill);

	*__pos = 0;
	{
		*buffer_ptr = glue_unpack(__pos, msg, ext, void*);
		if (*buffer_ptr) {
		}
...
int skb_copy_bits(struct sk_buff const* skb, int offset, void* to, int len)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*skb_ptr) {
			caller_marshal_kernel__skb_copy_bits__skb__io(__pos, msg, ext, ctx, *skb_ptr);
		}

	}

	{
		glue_pack(__pos, msg, ext, *offset_ptr);
	}
	...
	glue_call_server(__pos, msg, RPC_ID_skb_copy_bits);
	...
	*__pos = 0;
	{
		*skb_ptr = glue_unpack_shadow(__pos, msg, ext, struct sk_buff const*);
		if (*skb_ptr) {
			...
			caller_unmarshal_kernel__skb_copy_bits__skb__io(__pos, msg, ext, ctx, writable);
		}
...
int ___pskb_trim(struct sk_buff* skb, unsigned int len)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*skb_ptr) {
			caller_marshal_kernel_____pskb_trim__skb__io(__pos, msg, ext, ctx, *skb_ptr);
		}
	...
	glue_call_server(__pos, msg, RPC_ID____pskb_trim);

	*__pos = 0;
	{
		*skb_ptr = glue_unpack_shadow(__pos, msg, ext, struct sk_buff*);
		if (*skb_ptr) {
			caller_unmarshal_kernel_____pskb_trim__skb__io(__pos, msg, ext, ctx, *skb_ptr);
		}
...
int pskb_expand_head(struct sk_buff* skb, int nhead, int ntail, unsigned int gfp_mask)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*skb_ptr) {
			caller_marshal_kernel__pskb_expand_head__skb__io(__pos, msg, ext, ctx, *skb_ptr);
		}

	}

	{
		glue_pack(__pos, msg, ext, *nhead_ptr);
	}

	{
		glue_pack(__pos, msg, ext, *ntail_ptr);
	}
	...
	glue_call_server(__pos, msg, RPC_ID_pskb_expand_head);

	*__pos = 0;
	{
		*skb_ptr = glue_unpack_shadow(__pos, msg, ext, struct sk_buff*);
		if (*skb_ptr) {
			caller_unmarshal_kernel__pskb_expand_head__skb__io(__pos, msg, ext, ctx, *skb_ptr);
		}
...
unsigned char* __pskb_pull_tail(struct sk_buff* skb, int delta)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*skb_ptr) {
			caller_marshal_kernel____pskb_pull_tail__skb__io(__pos, msg, ext, ctx, *skb_ptr);
		}

	}

	{
		glue_pack(__pos, msg, ext, *delta_ptr);
	}

	glue_call_server(__pos, msg, RPC_ID___pskb_pull_tail);

	*__pos = 0;
	{
		*skb_ptr = glue_unpack_shadow(__pos, msg, ext, struct sk_buff*);
		if (*skb_ptr) {
			caller_unmarshal_kernel____pskb_pull_tail__skb__io(__pos, msg, ext, ctx, *skb_ptr);
		}
	...
	{
		*ret_ptr = glue_unpack_shadow(__pos, msg, ext, unsigned char*);
		if (*ret_ptr) {
...
void skb_tstamp_tx(struct sk_buff* orig_skb, struct skb_shared_hwtstamps* hwtstamps)
{
	...
	{
		const void* __adjusted = *orig_skb_ptr;
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*orig_skb_ptr) {
			caller_marshal_kernel__skb_tstamp_tx__orig_skb__io(__pos, msg, ext, ctx, *orig_skb_ptr);
		}

	}

	{
		const void* __adjusted = *hwtstamps_ptr;
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*hwtstamps_ptr) {
			caller_marshal_kernel__skb_tstamp_tx__hwtstamps__in(__pos, msg, ext, ctx, *hwtstamps_ptr);
		}

	}

	glue_call_server(__pos, msg, RPC_ID_skb_tstamp_tx);

	*__pos = 0;
	{
		if (*orig_skb_ptr) {
			caller_unmarshal_kernel__skb_tstamp_tx__orig_skb__io(__pos, msg, ext, ctx, *orig_skb_ptr);
		}
	...
	{
		if (*hwtstamps_ptr) {
			caller_unmarshal_kernel__skb_tstamp_tx__hwtstamps__in(__pos, msg, ext, ctx, *hwtstamps_ptr);
		}
...
int skb_pad(struct sk_buff* skb, int pad)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*skb_ptr) {
			caller_marshal_kernel__skb_pad__skb__io(__pos, msg, ext, ctx, *skb_ptr);
		}

	}

	{
		glue_pack(__pos, msg, ext, *pad_ptr);
	}

	glue_call_server(__pos, msg, RPC_ID_skb_pad);

	*__pos = 0;
	{
		*skb_ptr = glue_unpack_shadow(__pos, msg, ext, struct sk_buff*);
		if (*skb_ptr) {
			caller_unmarshal_kernel__skb_pad__skb__io(__pos, msg, ext, ctx, *skb_ptr);
		}
...
void napi_consume_skb(struct sk_buff* skb, int budget)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*skb_ptr) {
			caller_marshal_kernel__napi_consume_skb__skb__in(__pos, msg, ext, ctx, *skb_ptr);
		}
	...
	glue_call_server(__pos, msg, RPC_ID_napi_consume_skb);

	*__pos = 0;
	{
		if (*skb_ptr) {
			caller_unmarshal_kernel__napi_consume_skb__skb__in(__pos, msg, ext, ctx, *skb_ptr);
		}
...
struct sk_buff* __napi_alloc_skb(struct napi_struct* napi, unsigned int len, unsigned int gfp_mask)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*napi_ptr) {
			caller_marshal_kernel____napi_alloc_skb__napi__in(__pos, msg, ext, ctx, *napi_ptr);
		}
	...
	glue_call_server(__pos, msg, RPC_ID___napi_alloc_skb);

	*__pos = 0;
	{
		if (*napi_ptr) {
			caller_unmarshal_kernel____napi_alloc_skb__napi__in(__pos, msg, ext, ctx, *napi_ptr);
		}
	...
	{
		if (*ret_ptr) {
			caller_unmarshal_kernel____napi_alloc_skb__ret_sk_buff__in(__pos, msg, ext, ctx, *ret_ptr);
		}
...
void napi_gro_flush(struct napi_struct* napi, bool flush_old)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*napi_ptr) {
			caller_marshal_kernel__napi_gro_flush__napi__io(__pos, msg, ext, ctx, *napi_ptr);
		}

	}

	{
		glue_pack(__pos, msg, ext, *flush_old_ptr);
	}

	glue_call_server(__pos, msg, RPC_ID_napi_gro_flush);

	*__pos = 0;
	{
		*napi_ptr = glue_unpack_shadow(__pos, msg, ext, struct napi_struct*);
		if (*napi_ptr) {
			caller_unmarshal_kernel__napi_gro_flush__napi__io(__pos, msg, ext, ctx, *napi_ptr);
		}
...
void napi_complete_done(struct napi_struct* n, int work_done)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*n_ptr) {
			caller_marshal_kernel__napi_complete_done__n__io(__pos, msg, ext, ctx, *n_ptr);
		}
	...
	*__pos = 0;
	{
		*n_ptr = glue_unpack_shadow(__pos, msg, ext, struct napi_struct*);
		if (*n_ptr) {
			caller_unmarshal_kernel__napi_complete_done__n__io(__pos, msg, ext, ctx, *n_ptr);
		}
...
int call_netdevice_notifiers(unsigned long val, struct net_device* dev)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*dev_ptr) {
			caller_marshal_kernel__call_netdevice_notifiers__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}

	}

	glue_call_server(__pos, msg, RPC_ID_call_netdevice_notifiers);
	...
	*__pos = 0;
	...
	{
		if (*dev_ptr) {
			caller_unmarshal_kernel__call_netdevice_notifiers__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
...
struct rtnl_link_stats64* dev_get_stats(struct net_device* dev, struct rtnl_link_stats64* storage)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*dev_ptr) {
			caller_marshal_kernel__dev_get_stats__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}

	}

	{
		const void* __adjusted = *storage_ptr;
		glue_pack(__pos, msg, ext, __adjusted);
		if (*storage_ptr) {
			caller_marshal_kernel__dev_get_stats__storage__in(__pos, msg, ext, ctx, *storage_ptr);
		}

	}

	glue_call_server(__pos, msg, RPC_ID_dev_get_stats);

	*__pos = 0;
	{
		if (*dev_ptr) {
			caller_unmarshal_kernel__dev_get_stats__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
	...
	{
		if (*storage_ptr) {
			caller_unmarshal_kernel__dev_get_stats__storage__in(__pos, msg, ext, ctx, *storage_ptr);
		}
...

	{
		*ret_ptr = glue_unpack(__pos, msg, ext, struct rtnl_link_stats64*);
		if (*ret_ptr) {
			caller_unmarshal_kernel__dev_get_stats__ret_rtnl_link_stats64__out(__pos, msg, ext, ctx, *ret_ptr);
		}
...
void unregister_netdev(struct net_device* dev)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*dev_ptr) {
			caller_marshal_kernel__unregister_netdev__dev__io(__pos, msg, ext, ctx, *dev_ptr);
		}
	...
	*__pos = 0;
	{
		*dev_ptr = glue_unpack_shadow(__pos, msg, ext, struct net_device*);
		if (*dev_ptr) {
			caller_unmarshal_kernel__unregister_netdev__dev__io(__pos, msg, ext, ctx, *dev_ptr);
		}
...
void free_netdev(struct net_device* dev)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*dev_ptr) {
			caller_marshal_kernel__free_netdev__dev__io(__pos, msg, ext, ctx, *dev_ptr);
		}
	...
	*__pos = 0;
	{
		*dev_ptr = glue_unpack_shadow(__pos, msg, ext, struct net_device*);
		if (*dev_ptr) {
			caller_unmarshal_kernel__free_netdev__dev__io(__pos, msg, ext, ctx, *dev_ptr);
		}
...
bool napi_hash_del(struct napi_struct* napi)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*napi_ptr) {
			caller_marshal_kernel__napi_hash_del__napi__io(__pos, msg, ext, ctx, *napi_ptr);
		}

	}

	glue_call_server(__pos, msg, RPC_ID_napi_hash_del);

	*__pos = 0;
	{
		*napi_ptr = glue_unpack_shadow(__pos, msg, ext, struct napi_struct*);
		if (*napi_ptr) {
			caller_unmarshal_kernel__napi_hash_del__napi__io(__pos, msg, ext, ctx, *napi_ptr);
		}
...
void netif_tx_stop_all_queues(struct net_device* dev)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*dev_ptr) {
			caller_marshal_kernel__netif_tx_stop_all_queues__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}

	}

	glue_call_server(__pos, msg, RPC_ID_netif_tx_stop_all_queues);

	*__pos = 0;
	{
		if (*dev_ptr) {
			caller_unmarshal_kernel__netif_tx_stop_all_queues__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
...
void netif_carrier_on(struct net_device* dev)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*dev_ptr) {
			caller_marshal_kernel__netif_carrier_on__dev__io(__pos, msg, ext, ctx, *dev_ptr);
		}
	...
	*__pos = 0;
	{
		*dev_ptr = glue_unpack_shadow(__pos, msg, ext, struct net_device*);
		if (*dev_ptr) {
			caller_unmarshal_kernel__netif_carrier_on__dev__io(__pos, msg, ext, ctx, *dev_ptr);
		}
...
struct net_device* netdev_all_upper_get_next_dev_rcu(struct net_device* dev, struct list_head** iter)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*dev_ptr) {
			caller_marshal_kernel__netdev_all_upper_get_next_dev_rcu__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}

	}

	{
		const void* __adjusted = *iter_ptr;
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*iter_ptr) {
			const void* __adjusted = **iter_ptr;
			glue_pack_shadow(__pos, msg, ext, __adjusted);
			if (**iter_ptr) {
				caller_marshal_kernel__netdev_all_upper_get_next_dev_rcu__iter__in(__pos, msg, ext, ctx, **iter_ptr);
			}
	...
	glue_call_server(__pos, msg, RPC_ID_netdev_all_upper_get_next_dev_rcu);

	*__pos = 0;
	{
		if (*dev_ptr) {
			caller_unmarshal_kernel__netdev_all_upper_get_next_dev_rcu__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
...

	{
		if (*iter_ptr) {
			if (**iter_ptr) {
				caller_unmarshal_kernel__netdev_all_upper_get_next_dev_rcu__iter__in(__pos, msg, ext, ctx, **iter_ptr);
			}
	...
	{
		if (*ret_ptr) {
			caller_unmarshal_kernel__netdev_all_upper_get_next_dev_rcu__ret_net_device__in(__pos, msg, ext, ctx, *ret_ptr);
		}
...
void __napi_schedule_irqoff(struct napi_struct* n)
{
...

	{
		...
		glue_pack(__pos, msg, ext, __adjusted);
		if (*n_ptr) {
			caller_marshal_kernel____napi_schedule_irqoff__n__in(__pos, msg, ext, ctx, *n_ptr);
		}

	}

	glue_call_server(__pos, msg, RPC_ID___napi_schedule_irqoff);

	*__pos = 0;
	{
		if (*n_ptr) {
			caller_unmarshal_kernel____napi_schedule_irqoff__n__in(__pos, msg, ext, ctx, *n_ptr);
		}
...
unsigned int napi_gro_receive(struct napi_struct* napi, struct sk_buff* skb)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*skb_ptr) {
			caller_marshal_kernel__napi_gro_receive__skb__io(__pos, msg, ext, ctx, *skb_ptr);
		}
	...
	*__pos = 0;
...

	{
		*skb_ptr = glue_unpack_shadow(__pos, msg, ext, struct sk_buff*);
		if (*skb_ptr) {
			caller_unmarshal_kernel__napi_gro_receive__skb__io(__pos, msg, ext, ctx, *skb_ptr);
		}
...
int netif_receive_skb(struct sk_buff* skb)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*skb_ptr) {
			caller_marshal_kernel__netif_receive_skb__skb__io(__pos, msg, ext, ctx, *skb_ptr);
		}

	}

	glue_call_server(__pos, msg, RPC_ID_netif_receive_skb);

	*__pos = 0;
	{
		*skb_ptr = glue_unpack_shadow(__pos, msg, ext, struct sk_buff*);
		if (*skb_ptr) {
			caller_unmarshal_kernel__netif_receive_skb__skb__io(__pos, msg, ext, ctx, *skb_ptr);
		}
...
int skb_checksum_help(struct sk_buff* skb)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*skb_ptr) {
			caller_marshal_kernel__skb_checksum_help__skb__io(__pos, msg, ext, ctx, *skb_ptr);
		}

	}

	glue_call_server(__pos, msg, RPC_ID_skb_checksum_help);

	*__pos = 0;
	{
		*skb_ptr = glue_unpack_shadow(__pos, msg, ext, struct sk_buff*);
		if (*skb_ptr) {
			caller_unmarshal_kernel__skb_checksum_help__skb__io(__pos, msg, ext, ctx, *skb_ptr);
		}
...
void netif_tx_wake_queue(struct netdev_queue* dev_queue)
{
	...
	{
		const void* __adjusted = *dev_queue_ptr;
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*dev_queue_ptr) {
...
void netif_wake_subqueue(struct net_device* dev, unsigned short queue_index)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*dev_ptr) {
			caller_marshal_kernel__netif_wake_subqueue__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
	...
	glue_call_server(__pos, msg, RPC_ID_netif_wake_subqueue);

	*__pos = 0;
	{
		if (*dev_ptr) {
			caller_unmarshal_kernel__netif_wake_subqueue__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
...
int netif_set_real_num_rx_queues(struct net_device* dev, unsigned int rxq)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*dev_ptr) {
			caller_marshal_kernel__netif_set_real_num_rx_queues__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}

	}

	{
		glue_pack(__pos, msg, ext, *rxq_ptr);
	}

	glue_call_server(__pos, msg, RPC_ID_netif_set_real_num_rx_queues);

	*__pos = 0;
	{
		if (*dev_ptr) {
			caller_unmarshal_kernel__netif_set_real_num_rx_queues__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
...
int netif_set_real_num_tx_queues(struct net_device* dev, unsigned int txq)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*dev_ptr) {
			caller_marshal_kernel__netif_set_real_num_tx_queues__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}

	}

	{
		glue_pack(__pos, msg, ext, *txq_ptr);
	}

	glue_call_server(__pos, msg, RPC_ID_netif_set_real_num_tx_queues);

	*__pos = 0;
	{
		if (*dev_ptr) {
			caller_unmarshal_kernel__netif_set_real_num_tx_queues__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
...
int netif_set_xps_queue(struct net_device* dev, struct cpumask const* mask, unsigned short index)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*dev_ptr) {
			caller_marshal_kernel__netif_set_xps_queue__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}

	}

	{
		const void* __adjusted = *mask_ptr;
		glue_pack(__pos, msg, ext, __adjusted);
		if (*mask_ptr) {
			caller_marshal_kernel__netif_set_xps_queue__mask__in(__pos, msg, ext, ctx, *mask_ptr);
		}
	...
	glue_call_server(__pos, msg, RPC_ID_netif_set_xps_queue);

	*__pos = 0;
	{
		if (*dev_ptr) {
			caller_unmarshal_kernel__netif_set_xps_queue__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
...

	{
		if (*mask_ptr) {
			...
			caller_unmarshal_kernel__netif_set_xps_queue__mask__in(__pos, msg, ext, ctx, writable);
		}
...
int irq_set_affinity_hint(unsigned int irq, struct cpumask const* m)
{
	...
	{
		const void* __adjusted = *m_ptr;
		glue_pack(__pos, msg, ext, __adjusted);
		if (*m_ptr) {
			caller_marshal_kernel__irq_set_affinity_hint__m__in(__pos, msg, ext, ctx, *m_ptr);
		}

	}

	glue_call_server(__pos, msg, RPC_ID_irq_set_affinity_hint);
	...
	*__pos = 0;
...

	{
		if (*m_ptr) {
			...
			caller_unmarshal_kernel__irq_set_affinity_hint__m__in(__pos, msg, ext, ctx, writable);
		}
...
int pci_enable_msix_range(struct pci_dev* dev, struct msix_entry* entries, int minvec, int maxvec)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*dev_ptr) {
			caller_marshal_kernel__pci_enable_msix_range__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}

	}

	{
		const void* __adjusted = *entries_ptr;
		glue_pack(__pos, msg, ext, __adjusted);
		if (*entries_ptr) {
			...
			for (i = 0; i < len; ++i) {
				...
				caller_marshal_kernel__pci_enable_msix_range__entries__in(__pos, msg, ext, ctx, element);
			}
	...
	glue_call_server(__pos, msg, RPC_ID_pci_enable_msix_range);

	*__pos = 0;
	{
		if (*dev_ptr) {
			caller_unmarshal_kernel__pci_enable_msix_range__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
...
int dev_addr_del(struct net_device* dev, unsigned char const* addr, unsigned char addr_type)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*dev_ptr) {
			caller_marshal_kernel__dev_addr_del__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
	...
	{
		glue_pack(__pos, msg, ext, *addr_type_ptr);
	}

	glue_call_server(__pos, msg, RPC_ID_dev_addr_del);

	*__pos = 0;
	{
		if (*dev_ptr) {
			caller_unmarshal_kernel__dev_addr_del__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
...
int dev_addr_add(struct net_device* dev, unsigned char const* addr, unsigned char addr_type)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*dev_ptr) {
			caller_marshal_kernel__dev_addr_add__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
	...
	glue_call_server(__pos, msg, RPC_ID_dev_addr_add);

	*__pos = 0;
	{
		if (*dev_ptr) {
			caller_unmarshal_kernel__dev_addr_add__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
...
void sync_callee(struct fipc_message* msg, struct ext_registers* ext)
{
	size_t n_pos = 0;
	size_t* __pos = &n_pos;

	fptr_sync function_ptr = glue_unpack(__pos, msg, ext, fptr_sync);
	struct net_device* dev = 0;
...

	{
		*dev_ptr = glue_unpack_shadow(__pos, msg, ext, struct net_device*);
		if (*dev_ptr) {
			callee_unmarshal_kernel__sync__net_device__in(__pos, msg, ext, ctx, *dev_ptr);
		}
	...
	*__pos = 0;
	{
		if (*dev_ptr) {
			callee_marshal_kernel__sync__net_device__in(__pos, msg, ext, ctx, *dev_ptr);
		}
...
void unsync_callee(struct fipc_message* msg, struct ext_registers* ext)
{
	size_t n_pos = 0;
	size_t* __pos = &n_pos;

	fptr_unsync function_ptr = glue_unpack(__pos, msg, ext, fptr_unsync);
	struct net_device* dev = 0;
	unsigned char const* addr = 0;
	struct net_device** dev_ptr = &dev;
	unsigned char const** addr_ptr = &addr;
...

	{
		*dev_ptr = glue_unpack_shadow(__pos, msg, ext, struct net_device*);
		if (*dev_ptr) {
			callee_unmarshal_kernel__unsync__net_device__in(__pos, msg, ext, ctx, *dev_ptr);
		}
	...
	*__pos = 0;
	{
		if (*dev_ptr) {
			callee_marshal_kernel__unsync__net_device__in(__pos, msg, ext, ctx, *dev_ptr);
		}
...
void __hw_addr_unsync_dev(struct netdev_hw_addr_list* list, struct net_device* dev, fptr_unsync unsync)
{
	...
	{
		const void* __adjusted = *list_ptr;
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*list_ptr) {
			caller_marshal_kernel____hw_addr_unsync_dev__list__in(__pos, msg, ext, ctx, *list_ptr);
		}
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*dev_ptr) {
			caller_marshal_kernel____hw_addr_unsync_dev__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}

	}

	{
		glue_pack(__pos, msg, ext, *unsync_ptr);
	}

	glue_call_server(__pos, msg, RPC_ID___hw_addr_unsync_dev);

	*__pos = 0;
	{
		if (*list_ptr) {
			caller_unmarshal_kernel____hw_addr_unsync_dev__list__in(__pos, msg, ext, ctx, *list_ptr);
		}
	...
	{
		if (*dev_ptr) {
			caller_unmarshal_kernel____hw_addr_unsync_dev__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
...
int __hw_addr_sync_dev(struct netdev_hw_addr_list* list, struct net_device* dev, fptr_sync sync, fptr_unsync unsync)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*list_ptr) {
			caller_marshal_kernel____hw_addr_sync_dev__list__in(__pos, msg, ext, ctx, *list_ptr);
		}
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*dev_ptr) {
			caller_marshal_kernel____hw_addr_sync_dev__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
	...
	glue_call_server(__pos, msg, RPC_ID___hw_addr_sync_dev);

	*__pos = 0;
	{
		if (*list_ptr) {
			caller_unmarshal_kernel____hw_addr_sync_dev__list__in(__pos, msg, ext, ctx, *list_ptr);
		}
	...
	{
		if (*dev_ptr) {
			caller_unmarshal_kernel____hw_addr_sync_dev__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
...
int eth_platform_get_mac_address(struct device* dev, unsigned char* mac_addr)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*dev_ptr) {
			caller_marshal_kernel__eth_platform_get_mac_address__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}

	}

	{
		const void* __adjusted = *mac_addr_ptr;
		glue_pack(__pos, msg, ext, __adjusted);
		(void)mac_addr_ptr;
	}

	glue_call_server(__pos, msg, RPC_ID_eth_platform_get_mac_address);

	*__pos = 0;
	{
		if (*dev_ptr) {
			caller_unmarshal_kernel__eth_platform_get_mac_address__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
...
void ndo_validate_addr_callee(struct fipc_message* msg, struct ext_registers* ext)
{
	size_t n_pos = 0;
	size_t* __pos = &n_pos;

	fptr_ndo_validate_addr function_ptr = glue_unpack(__pos, msg, ext, fptr_ndo_validate_addr);
	struct net_device* dev = 0;
	struct net_device** dev_ptr = &dev;
	int ret = 0;
	int* ret_ptr = &ret;
...

	{
		*dev_ptr = glue_unpack_shadow(__pos, msg, ext, struct net_device*);
		if (*dev_ptr) {
			callee_unmarshal_kernel__ndo_validate_addr__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
	...
	*__pos = 0;
	{
		if (*dev_ptr) {
			callee_marshal_kernel__ndo_validate_addr__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
...
unsigned long dev_trans_start(struct net_device* dev)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*dev_ptr) {
			caller_marshal_kernel__dev_trans_start__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}

	}

	glue_call_server(__pos, msg, RPC_ID_dev_trans_start);

	*__pos = 0;
	{
		if (*dev_ptr) {
			caller_unmarshal_kernel__dev_trans_start__dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
...
int mdio_mii_ioctl(struct mdio_if_info const* mdio, struct mii_ioctl_data* mii_data, int cmd)
{
	...
	{
		const void* __adjusted = *mdio_ptr;
		glue_pack(__pos, msg, ext, __adjusted);
		if (*mdio_ptr) {
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*mii_data_ptr) {
			caller_marshal_kernel__mdio_mii_ioctl__mii_data__io(__pos, msg, ext, ctx, *mii_data_ptr);
		}
	...
	*__pos = 0;
...

	{
		*mii_data_ptr = glue_unpack_shadow(__pos, msg, ext, struct mii_ioctl_data*);
		if (*mii_data_ptr) {
			caller_unmarshal_kernel__mdio_mii_ioctl__mii_data__io(__pos, msg, ext, ctx, *mii_data_ptr);
		}
...
int mdio45_probe(struct mdio_if_info* mdio, int prtad)
{
...

	{
		...
		glue_pack(__pos, msg, ext, __adjusted);
		if (*mdio_ptr) {
			caller_marshal_kernel__mdio45_probe__mdio__in(__pos, msg, ext, ctx, *mdio_ptr);
		}
	...
	glue_call_server(__pos, msg, RPC_ID_mdio45_probe);

	*__pos = 0;
	{
		if (*mdio_ptr) {
			caller_unmarshal_kernel__mdio45_probe__mdio__in(__pos, msg, ext, ctx, *mdio_ptr);
		}
...
int try_dispatch(enum RPC_ID id, struct fipc_message* msg, struct ext_registers* ext)
{
	...
	case RPC_ID_ndo_stop:
		...
		ndo_stop_callee(msg, ext);
		break;

	case RPC_ID_ndo_open:
		glue_user_trace("ndo_open\n");
		ndo_open_callee(msg, ext);
		break;
