	...
	struct pci_dev const* ptr)
{
	...
	
	{
		...
		if (*bus_ptr) {
			caller_marshal_kernel__probe__pci_dev_bus__in(__pos, msg, ext, ctx, *bus_ptr);
		}
...

	{
		...
		for (i = 0; i < len; ++i) {
			...
			caller_marshal_kernel__probe__resource__in(__pos, msg, ext, ctx, element);
		}
	...
	struct pci_dev* ptr)
{
	...
	
	{
		*bus_ptr = glue_unpack_new_shadow(__pos, msg, ext, struct pci_bus*, (sizeof(struct pci_bus)), (DEFAULT_GFP_FLAGS));
		if (*bus_ptr) {
			callee_unmarshal_kernel__probe__pci_dev_bus__in(__pos, msg, ext, ctx, *bus_ptr);
		}
...

	{
		...
		for (i = 0; i < len; ++i) {
			...
			callee_unmarshal_kernel__probe__resource__in(__pos, msg, ext, ctx, element);
		}
	...
	struct pci_dev const* ptr)
{
	...
	{
		if (*bus_ptr) {
			callee_marshal_kernel__probe__pci_dev_bus__in(__pos, msg, ext, ctx, *bus_ptr);
		}
	...
	struct pci_dev* ptr)
{
	...
	{
		if (*bus_ptr) {
			caller_unmarshal_kernel__probe__pci_dev_bus__in(__pos, msg, ext, ctx, *bus_ptr);
		}
	...
	struct device const* ptr)
{
	...
	{
		caller_marshal_kernel__probe__kobject__in(__pos, msg, ext, ctx, kobj_ptr);
	}
	...
	struct device* ptr)
{
	...
	{
		callee_unmarshal_kernel__probe__kobject__in(__pos, msg, ext, ctx, kobj_ptr);
	}
	...
	struct device const* ptr)
{
	...
	{
		callee_marshal_kernel__probe__kobject__in(__pos, msg, ext, ctx, kobj_ptr);
	}
	...
	struct device* ptr)
{
	...
	{
		caller_unmarshal_kernel__probe__kobject__in(__pos, msg, ext, ctx, kobj_ptr);
	}
	...
	struct kobject* ptr)
{
	...
	{
		*name_ptr = glue_unpack(__pos, msg, ext, char const*);
		if (*name_ptr) {
	...
	struct net_device* ptr)
{
	...
	{
		*state_ptr = glue_unpack(__pos, msg, ext, unsigned long long);
	}
	...
	struct net_device const* ptr)
{
	...
	{
		const void* __adjusted = *_tx_ptr;
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*_tx_ptr) {
			caller_marshal_kernel__ndo_open__net_device__tx__in(__pos, msg, ext, ctx, *_tx_ptr);
		}

	}

	{
		glue_pack(__pos, msg, ext, *num_tc_ptr);
	}
	...
	struct net_device* ptr)
{
	...
	{
		*_tx_ptr = glue_unpack(__pos, msg, ext, struct netdev_queue*);
		if (*_tx_ptr) {
			callee_unmarshal_kernel__ndo_open__net_device__tx__in(__pos, msg, ext, ctx, *_tx_ptr);
		}

	}

	{
		*num_tc_ptr = glue_unpack(__pos, msg, ext, unsigned char);
	}
	...
	struct net_device const* ptr)
{
	...
	{
		glue_pack(__pos, msg, ext, *num_rx_queues_ptr);
	}

	{
		if (*_tx_ptr) {
			callee_marshal_kernel__ndo_open__net_device__tx__in(__pos, msg, ext, ctx, *_tx_ptr);
		}
	...
	struct net_device* ptr)
{
	...
	{
		*num_rx_queues_ptr = glue_unpack(__pos, msg, ext, unsigned int);
	}

	{
		if (*_tx_ptr) {
			caller_unmarshal_kernel__ndo_open__net_device__tx__in(__pos, msg, ext, ctx, *_tx_ptr);
		}
	...
	struct sk_buff const* ptr)
{
	...
	{
		glue_pack(__pos, msg, ext, *mac_len_ptr);
	}

	{
		glue_pack(__pos, msg, ext, *queue_mapping_ptr);
	}
	...
	{
		glue_pack(__pos, msg, ext, *priority_ptr);
	}

	{
		glue_pack(__pos, msg, ext, *vlan_tci_ptr);
	}

	{
		glue_pack(__pos, msg, ext, *inner_network_header_ptr);
	}
	...
	{
		glue_pack(__pos, msg, ext, *transport_header_ptr);
	}

	{
		glue_pack(__pos, msg, ext, *network_header_ptr);
	}
	...
	{
		const void* __adjusted = *head_ptr;
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*head_ptr) {
	...
	struct sk_buff* ptr)
{
	...
	
	{
		...
		for (i = 0; i < len; ++i) {
			...
			*element = glue_unpack(__pos, msg, ext, signed char);
		}
	...
	{
		*mac_len_ptr = glue_unpack(__pos, msg, ext, unsigned short);
	}

	{
		*queue_mapping_ptr = glue_unpack(__pos, msg, ext, unsigned short);
	}
	...
	{
		*priority_ptr = glue_unpack(__pos, msg, ext, unsigned int);
	}

	{
		*vlan_tci_ptr = glue_unpack(__pos, msg, ext, unsigned short);
	}

	{
		*inner_network_header_ptr = glue_unpack(__pos, msg, ext, unsigned short);
	}
	...
	{
		*transport_header_ptr = glue_unpack(__pos, msg, ext, unsigned short);
	}

	{
		*network_header_ptr = glue_unpack(__pos, msg, ext, unsigned short);
	}
	...
	struct net_device const* ptr)
{
	...
	{
		const void* __adjusted = *dev_addr_ptr;
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		(void)dev_addr_ptr;
	}
	...
	struct rtnl_link_stats64 const* ptr)
{
	...
	{
		glue_pack(__pos, msg, ext, *rx_missed_errors_ptr);
	}
	...
	struct rtnl_link_stats64* ptr)
{
	...
	{
		*rx_missed_errors_ptr = glue_unpack(__pos, msg, ext, unsigned long long);
	}
	...
	struct net_device const* ptr)
{
...

	{
		...
		for (i = 0; i < len; ++i) {
			...
			callee_marshal_kernel__ndo_setup_tc__netdev_tc_txq__out(__pos, msg, ext, ctx, element);
		}
	...
	struct net_device* ptr)
{
...

	{
		...
		for (i = 0; i < len; ++i) {
			...
			caller_unmarshal_kernel__ndo_setup_tc__netdev_tc_txq__out(__pos, msg, ext, ctx, element);
		}
	...
	struct pci_dev const* ptr)
{
	...
	
	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*bus_ptr) {
			caller_marshal_kernel__pci_disable_msix__pci_dev_bus__in(__pos, msg, ext, ctx, *bus_ptr);
		}
	...
	struct pci_dev* ptr)
{
	...
	
	{
		*bus_ptr = glue_unpack(__pos, msg, ext, struct pci_bus*);
		if (*bus_ptr) {
			callee_unmarshal_kernel__pci_disable_msix__pci_dev_bus__in(__pos, msg, ext, ctx, *bus_ptr);
		}
	...
	struct pci_dev const* ptr)
{
	...
	{
		if (*bus_ptr) {
			callee_marshal_kernel__pci_disable_msix__pci_dev_bus__in(__pos, msg, ext, ctx, *bus_ptr);
		}
	...
	struct pci_dev* ptr)
{
	...
	{
		if (*bus_ptr) {
			caller_unmarshal_kernel__pci_disable_msix__pci_dev_bus__in(__pos, msg, ext, ctx, *bus_ptr);
		}
	...
	struct pci_dev const* ptr)
{
	...
	
	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*bus_ptr) {
			caller_marshal_kernel__pci_disable_device__pci_dev_bus__in(__pos, msg, ext, ctx, *bus_ptr);
		}
	...
	struct pci_dev* ptr)
{
	...
	
	{
		*bus_ptr = glue_unpack(__pos, msg, ext, struct pci_bus*);
		if (*bus_ptr) {
			callee_unmarshal_kernel__pci_disable_device__pci_dev_bus__in(__pos, msg, ext, ctx, *bus_ptr);
		}
	...
	struct pci_dev const* ptr)
{
	...
	{
		if (*bus_ptr) {
			callee_marshal_kernel__pci_disable_device__pci_dev_bus__in(__pos, msg, ext, ctx, *bus_ptr);
		}
	...
	struct pci_dev* ptr)
{
	...
	{
		if (*bus_ptr) {
			caller_unmarshal_kernel__pci_disable_device__pci_dev_bus__in(__pos, msg, ext, ctx, *bus_ptr);
		}
	...
	struct work_struct const* ptr)
{
	...
	{
		caller_marshal_kernel__queue_work_on__list_head__in(__pos, msg, ext, ctx, entry_ptr);
	}
	...
	struct work_struct* ptr)
{
	...
	{
		callee_unmarshal_kernel__queue_work_on__list_head__in(__pos, msg, ext, ctx, entry_ptr);
	}
	...
	struct work_struct const* ptr)
{
	...
	{
		callee_marshal_kernel__queue_work_on__list_head__in(__pos, msg, ext, ctx, entry_ptr);
	}
	...
	struct work_struct* ptr)
{
	...
	{
		caller_unmarshal_kernel__queue_work_on__list_head__in(__pos, msg, ext, ctx, entry_ptr);
	}
	...
	struct pci_dev const* ptr)
{
	...
	
	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*bus_ptr) {
			caller_marshal_kernel__pcie_get_minimum_link__pci_dev_bus__in(__pos, msg, ext, ctx, *bus_ptr);
		}
	...
	struct pci_dev* ptr)
{
	...
	
	{
		*bus_ptr = glue_unpack(__pos, msg, ext, struct pci_bus*);
		if (*bus_ptr) {
			callee_unmarshal_kernel__pcie_get_minimum_link__pci_dev_bus__in(__pos, msg, ext, ctx, *bus_ptr);
		}
	...
	struct pci_dev const* ptr)
{
	...
	{
		if (*bus_ptr) {
			callee_marshal_kernel__pcie_get_minimum_link__pci_dev_bus__in(__pos, msg, ext, ctx, *bus_ptr);
		}
	...
	struct pci_dev* ptr)
{
	...
	{
		if (*bus_ptr) {
			caller_unmarshal_kernel__pcie_get_minimum_link__pci_dev_bus__in(__pos, msg, ext, ctx, *bus_ptr);
		}
	...
	struct pci_dev const* ptr)
{
	...
	
	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*bus_ptr) {
			caller_marshal_kernel__pcie_capability_read_word__pci_dev_bus__in(__pos, msg, ext, ctx, *bus_ptr);
		}

	}

	{
		glue_pack(__pos, msg, ext, *pcie_cap_ptr);
	}
	...
	struct pci_dev* ptr)
{
	...
	
	{
		*bus_ptr = glue_unpack(__pos, msg, ext, struct pci_bus*);
		if (*bus_ptr) {
			callee_unmarshal_kernel__pcie_capability_read_word__pci_dev_bus__in(__pos, msg, ext, ctx, *bus_ptr);
		}

	}

	{
		*pcie_cap_ptr = glue_unpack(__pos, msg, ext, unsigned char);
	}
	...
	struct pci_dev const* ptr)
{
	...
	{
		if (*bus_ptr) {
			callee_marshal_kernel__pcie_capability_read_word__pci_dev_bus__in(__pos, msg, ext, ctx, *bus_ptr);
		}
	...
	struct pci_dev* ptr)
{
	...
	{
		if (*bus_ptr) {
			caller_unmarshal_kernel__pcie_capability_read_word__pci_dev_bus__in(__pos, msg, ext, ctx, *bus_ptr);
		}
	...
	struct napi_struct const* ptr)
{
	...
	{
		const void* __adjusted = *gro_list_ptr;
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*gro_list_ptr) {
			caller_marshal_kernel____napi_alloc_skb__napi_struct_gro_list__in(__pos, msg, ext, ctx, *gro_list_ptr);
		}
	...
	struct napi_struct* ptr)
{
	...
	{
		*gro_list_ptr = glue_unpack(__pos, msg, ext, struct sk_buff*);
		if (*gro_list_ptr) {
			callee_unmarshal_kernel____napi_alloc_skb__napi_struct_gro_list__in(__pos, msg, ext, ctx, *gro_list_ptr);
		}
	...
	struct napi_struct const* ptr)
{
	...
	{
		if (*gro_list_ptr) {
			callee_marshal_kernel____napi_alloc_skb__napi_struct_gro_list__in(__pos, msg, ext, ctx, *gro_list_ptr);
		}
	...
	struct napi_struct* ptr)
{
	...
	{
		if (*gro_list_ptr) {
			caller_unmarshal_kernel____napi_alloc_skb__napi_struct_gro_list__in(__pos, msg, ext, ctx, *gro_list_ptr);
		}
	...
	struct napi_struct const* ptr)
{
	...
	
	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*gro_list_ptr) {
			caller_marshal_kernel__napi_gro_flush__napi_struct_gro_list__io(__pos, msg, ext, ctx, *gro_list_ptr);
		}
	...
	struct napi_struct* ptr)
{
	...
	{
		*gro_list_ptr = glue_unpack_shadow(__pos, msg, ext, struct sk_buff*);
		if (*gro_list_ptr) {
			callee_unmarshal_kernel__napi_gro_flush__napi_struct_gro_list__io(__pos, msg, ext, ctx, *gro_list_ptr);
		}
	...
	struct napi_struct const* ptr)
{
	...
	
	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*gro_list_ptr) {
			callee_marshal_kernel__napi_gro_flush__napi_struct_gro_list__io(__pos, msg, ext, ctx, *gro_list_ptr);
		}
	...
	struct napi_struct* ptr)
{
	...
	
	{
		*gro_list_ptr = glue_unpack_shadow(__pos, msg, ext, struct sk_buff*);
		if (*gro_list_ptr) {
			caller_unmarshal_kernel__napi_gro_flush__napi_struct_gro_list__io(__pos, msg, ext, ctx, *gro_list_ptr);
		}
	...
	struct sk_buff const* ptr)
{
	...
	{
		glue_pack(__pos, msg, ext, *hash_ptr);
	}

	{
		glue_pack(__pos, msg, ext, *vlan_proto_ptr);
	}
	...
	struct sk_buff* ptr)
{
	...
	{
		*hash_ptr = glue_unpack(__pos, msg, ext, unsigned int);
	}

	{
		*vlan_proto_ptr = glue_unpack(__pos, msg, ext, unsigned short);
	}
	...
	struct napi_struct const* ptr)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*gro_list_ptr) {
			caller_marshal_kernel__napi_complete_done__napi_struct_gro_list__io(__pos, msg, ext, ctx, *gro_list_ptr);
		}
	...
	struct napi_struct* ptr)
{
...

	{
		*gro_list_ptr = glue_unpack_shadow(__pos, msg, ext, struct sk_buff*);
		if (*gro_list_ptr) {
			callee_unmarshal_kernel__napi_complete_done__napi_struct_gro_list__io(__pos, msg, ext, ctx, *gro_list_ptr);
		}
	...
	struct napi_struct const* ptr)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*gro_list_ptr) {
			callee_marshal_kernel__napi_complete_done__napi_struct_gro_list__io(__pos, msg, ext, ctx, *gro_list_ptr);
		}
	...
	struct napi_struct* ptr)
{
...

	{
		*gro_list_ptr = glue_unpack_shadow(__pos, msg, ext, struct sk_buff*);
		if (*gro_list_ptr) {
			caller_unmarshal_kernel__napi_complete_done__napi_struct_gro_list__io(__pos, msg, ext, ctx, *gro_list_ptr);
		}
	...
	struct net_device const* ptr)
{
	...
	
	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*dev_addr_ptr) {
			glue_pack(__pos, msg, ext, **dev_addr_ptr);
		}
	...
	struct napi_struct const* ptr)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*gro_list_ptr) {
			caller_marshal_kernel__netif_napi_del__napi_struct_gro_list__in(__pos, msg, ext, ctx, *gro_list_ptr);
		}
	...
	struct napi_struct* ptr)
{
...

	{
		*gro_list_ptr = glue_unpack(__pos, msg, ext, struct sk_buff*);
		if (*gro_list_ptr) {
			callee_unmarshal_kernel__netif_napi_del__napi_struct_gro_list__in(__pos, msg, ext, ctx, *gro_list_ptr);
		}
	...
	struct napi_struct const* ptr)
{
	...
	{
		if (*gro_list_ptr) {
			callee_marshal_kernel__netif_napi_del__napi_struct_gro_list__in(__pos, msg, ext, ctx, *gro_list_ptr);
		}
	...
	struct napi_struct* ptr)
{
	...
	{
		if (*gro_list_ptr) {
			caller_unmarshal_kernel__netif_napi_del__napi_struct_gro_list__in(__pos, msg, ext, ctx, *gro_list_ptr);
		}
	...
	struct napi_struct const* ptr)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*gro_list_ptr) {
			caller_marshal_kernel__napi_hash_del__napi_struct_gro_list__io(__pos, msg, ext, ctx, *gro_list_ptr);
		}
	...
	struct napi_struct* ptr)
{
...

	{
		*gro_list_ptr = glue_unpack_shadow(__pos, msg, ext, struct sk_buff*);
		if (*gro_list_ptr) {
			callee_unmarshal_kernel__napi_hash_del__napi_struct_gro_list__io(__pos, msg, ext, ctx, *gro_list_ptr);
		}
	...
	struct napi_struct const* ptr)
{
	...
	
	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*gro_list_ptr) {
			callee_marshal_kernel__napi_hash_del__napi_struct_gro_list__io(__pos, msg, ext, ctx, *gro_list_ptr);
		}
	...
	struct napi_struct* ptr)
{
	...
	
	{
		*gro_list_ptr = glue_unpack_shadow(__pos, msg, ext, struct sk_buff*);
		if (*gro_list_ptr) {
			caller_unmarshal_kernel__napi_hash_del__napi_struct_gro_list__io(__pos, msg, ext, ctx, *gro_list_ptr);
		}
	...
	struct net_device const* ptr)
{
	...
	{
		glue_pack(__pos, msg, ext, *vlan_features_ptr);
	}

	{
		glue_pack(__pos, msg, ext, *hw_enc_features_ptr);
	}

	{
		glue_pack(__pos, msg, ext, *mpls_features_ptr);
	}

	{
		const void* __adjusted = *netdev_ops_ptr;
		glue_pack(__pos, msg, ext, __adjusted);
		if (*netdev_ops_ptr) {
			caller_marshal_kernel__net_device__global_netdev_ops__in(__pos, msg, ext, *netdev_ops_ptr);
		}

	}

	{
		const void* __adjusted = *ethtool_ops_ptr;
		glue_pack(__pos, msg, ext, __adjusted);
		if (*ethtool_ops_ptr) {
			caller_marshal_kernel__net_device__global_ethtool_ops__in(__pos, msg, ext, *ethtool_ops_ptr);
		}
	...
	struct net_device* ptr)
{
	...
	{
		*vlan_features_ptr = glue_unpack(__pos, msg, ext, unsigned long long);
	}

	{
		*hw_enc_features_ptr = glue_unpack(__pos, msg, ext, unsigned long long);
	}

	{
		*mpls_features_ptr = glue_unpack(__pos, msg, ext, unsigned long long);
	}

	{
		*netdev_ops_ptr = glue_unpack_new_shadow(__pos, msg, ext, struct net_device_ops*, (sizeof(struct net_device_ops)), (DEFAULT_GFP_FLAGS));
		if (*netdev_ops_ptr) {
			callee_unmarshal_kernel__net_device__global_netdev_ops__in(__pos, msg, ext, *netdev_ops_ptr);
		}
...

	{
		*ethtool_ops_ptr = glue_unpack_new_shadow(__pos, msg, ext, struct ethtool_ops*, (sizeof(struct ethtool_ops)), (DEFAULT_GFP_FLAGS));
		if (*ethtool_ops_ptr) {
			callee_unmarshal_kernel__net_device__global_ethtool_ops__in(__pos, msg, ext, *ethtool_ops_ptr);
		}
	...
	struct net_device const* ptr)
{
	...
	{
		if (*netdev_ops_ptr) {
			callee_marshal_kernel__net_device__global_netdev_ops__in(__pos, msg, ext, *netdev_ops_ptr);
		}
	...
	{
		if (*ethtool_ops_ptr) {
			callee_marshal_kernel__net_device__global_ethtool_ops__in(__pos, msg, ext, *ethtool_ops_ptr);
		}
	...
	struct net_device* ptr)
{
	...
	{
		if (*netdev_ops_ptr) {
			caller_unmarshal_kernel__net_device__global_netdev_ops__in(__pos, msg, ext, *netdev_ops_ptr);
		}
	...
	{
		if (*ethtool_ops_ptr) {
			caller_unmarshal_kernel__net_device__global_ethtool_ops__in(__pos, msg, ext, *ethtool_ops_ptr);
		}
	...
	struct net_device_ops const* ptr)
{
	...
	{
		glue_pack(__pos, msg, ext, *ndo_features_check_ptr);
	}
	...
	{
		glue_pack(__pos, msg, ext, *ndo_set_features_ptr);
	}
	...
	{
		glue_pack(__pos, msg, ext, *ndo_set_tx_maxrate_ptr);
	}

	{
		glue_pack(__pos, msg, ext, *ndo_setup_tc_ptr);
	}
	...
	struct net_device_ops* ptr)
{
	...
	{
		*ndo_change_mtu_ptr = glue_unpack_rpc_ptr(__pos, msg, ext, ndo_change_mtu);
	}

	{
		*ndo_do_ioctl_ptr = glue_unpack_rpc_ptr(__pos, msg, ext, ndo_do_ioctl);
	}

	{
		*ndo_features_check_ptr = glue_unpack_rpc_ptr(__pos, msg, ext, ndo_features_check);
	}

	{
		*ndo_fix_features_ptr = glue_unpack_rpc_ptr(__pos, msg, ext, ndo_fix_features);
	}

	{
		*ndo_get_stats64_ptr = glue_unpack_rpc_ptr(__pos, msg, ext, ndo_get_stats64);
	}

	{
		*ndo_open_ptr = glue_unpack_rpc_ptr(__pos, msg, ext, ndo_open);
	}

	{
		*ndo_set_features_ptr = glue_unpack_rpc_ptr(__pos, msg, ext, ndo_set_features);
	}

	{
		*ndo_set_mac_address_ptr = glue_unpack_rpc_ptr(__pos, msg, ext, ndo_set_mac_address);
	}

	{
		*ndo_set_rx_mode_ptr = glue_unpack_rpc_ptr(__pos, msg, ext, ndo_set_rx_mode);
	}

	{
		*ndo_set_tx_maxrate_ptr = glue_unpack_rpc_ptr(__pos, msg, ext, ndo_set_tx_maxrate);
	}

	{
		*ndo_setup_tc_ptr = glue_unpack_rpc_ptr(__pos, msg, ext, ndo_setup_tc);
	}

	{
		*ndo_start_xmit_ptr = glue_unpack_rpc_ptr(__pos, msg, ext, ndo_start_xmit);
	}

	{
		*ndo_stop_ptr = glue_unpack_rpc_ptr(__pos, msg, ext, ndo_stop);
	}

	{
		*ndo_tx_timeout_ptr = glue_unpack_rpc_ptr(__pos, msg, ext, ndo_tx_timeout);
	}
	...
	struct napi_struct const* ptr)
{
	...
	{
		glue_pack(__pos, msg, ext, *napi_id_ptr);
	}
	...
	struct napi_struct* ptr)
{
	...
	{
		*napi_id_ptr = glue_unpack(__pos, msg, ext, unsigned int);
	}
	...
	struct napi_struct const* ptr)
{
	...
	
	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*gro_list_ptr) {
			caller_marshal_kernel__napi_gro_receive__napi_struct_gro_list__in(__pos, msg, ext, ctx, *gro_list_ptr);
		}
	...
	struct napi_struct* ptr)
{
	...
	
	{
		*gro_list_ptr = glue_unpack(__pos, msg, ext, struct sk_buff*);
		if (*gro_list_ptr) {
			callee_unmarshal_kernel__napi_gro_receive__napi_struct_gro_list__in(__pos, msg, ext, ctx, *gro_list_ptr);
		}
	...
	struct napi_struct const* ptr)
{
	...
	{
		if (*gro_list_ptr) {
			callee_marshal_kernel__napi_gro_receive__napi_struct_gro_list__in(__pos, msg, ext, ctx, *gro_list_ptr);
		}
	...
	struct napi_struct* ptr)
{
	...
	{
		if (*gro_list_ptr) {
			caller_unmarshal_kernel__napi_gro_receive__napi_struct_gro_list__in(__pos, msg, ext, ctx, *gro_list_ptr);
		}
	...
	struct pci_dev const* ptr)
{
	...
	
	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*bus_ptr) {
			caller_marshal_kernel__pci_enable_msix_range__pci_dev_bus__in(__pos, msg, ext, ctx, *bus_ptr);
		}
	...
	struct pci_dev* ptr)
{
	...
	
	{
		*bus_ptr = glue_unpack(__pos, msg, ext, struct pci_bus*);
		if (*bus_ptr) {
			callee_unmarshal_kernel__pci_enable_msix_range__pci_dev_bus__in(__pos, msg, ext, ctx, *bus_ptr);
		}
	...
	struct pci_dev const* ptr)
{
	...
	{
		if (*bus_ptr) {
			callee_marshal_kernel__pci_enable_msix_range__pci_dev_bus__in(__pos, msg, ext, ctx, *bus_ptr);
		}
	...
	struct pci_dev* ptr)
{
	...
	{
		if (*bus_ptr) {
			caller_unmarshal_kernel__pci_enable_msix_range__pci_dev_bus__in(__pos, msg, ext, ctx, *bus_ptr);
		}
	...
	struct netdev_hw_addr_list const* ptr)
{
	...
	{
		caller_marshal_kernel____hw_addr_unsync_dev__list_head__in(__pos, msg, ext, ctx, list_ptr);
	}
	...
	struct netdev_hw_addr_list* ptr)
{
	...
	{
		callee_unmarshal_kernel____hw_addr_unsync_dev__list_head__in(__pos, msg, ext, ctx, list_ptr);
	}
	...
	struct netdev_hw_addr_list const* ptr)
{
	...
	{
		callee_marshal_kernel____hw_addr_unsync_dev__list_head__in(__pos, msg, ext, ctx, list_ptr);
	}
	...
	struct netdev_hw_addr_list* ptr)
{
	...
	{
		caller_unmarshal_kernel____hw_addr_unsync_dev__list_head__in(__pos, msg, ext, ctx, list_ptr);
	}
	...
	struct list_head const* ptr)
{
	...
	
	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*next_ptr) {
			caller_marshal_kernel____hw_addr_unsync_dev__list_head_next__in(__pos, msg, ext, ctx, *next_ptr);
		}
	...
	struct list_head* ptr)
{
	...
	
	{
		*next_ptr = glue_unpack(__pos, msg, ext, struct list_head*);
		if (*next_ptr) {
			callee_unmarshal_kernel____hw_addr_unsync_dev__list_head_next__in(__pos, msg, ext, ctx, *next_ptr);
		}
	...
	struct list_head const* ptr)
{
	...
	{
		if (*next_ptr) {
			callee_marshal_kernel____hw_addr_unsync_dev__list_head_next__in(__pos, msg, ext, ctx, *next_ptr);
		}
	...
	struct list_head* ptr)
{
	...
	{
		if (*next_ptr) {
			caller_unmarshal_kernel____hw_addr_unsync_dev__list_head_next__in(__pos, msg, ext, ctx, *next_ptr);
		}
	...
	struct net_device const* ptr)
{
	...
	{
		glue_pack(__pos, msg, ext, *priv_flags_ptr);
	}
	...
	struct net_device* ptr)
{
	...
	{
		*priv_flags_ptr = glue_unpack(__pos, msg, ext, unsigned int);
	}
	...
	struct mdio_if_info const* ptr)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*dev_ptr) {
			caller_marshal_kernel__mdio_mii_ioctl__mdio_if_info_dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
	...
	struct mdio_if_info* ptr)
{
...

	{
		*dev_ptr = glue_unpack(__pos, msg, ext, struct net_device*);
		if (*dev_ptr) {
			callee_unmarshal_kernel__mdio_mii_ioctl__mdio_if_info_dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
	...
	struct mdio_if_info const* ptr)
{
	...
	{
		if (*dev_ptr) {
			callee_marshal_kernel__mdio_mii_ioctl__mdio_if_info_dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
	...
	struct mdio_if_info* ptr)
{
	...
	{
		if (*dev_ptr) {
			caller_unmarshal_kernel__mdio_mii_ioctl__mdio_if_info_dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
	...
	struct mdio_if_info const* ptr)
{
...

	{
		...
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*dev_ptr) {
			caller_marshal_kernel__mdio45_probe__mdio_if_info_dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
	...
	struct mdio_if_info* ptr)
{
...

	{
		*dev_ptr = glue_unpack(__pos, msg, ext, struct net_device*);
		if (*dev_ptr) {
			callee_unmarshal_kernel__mdio45_probe__mdio_if_info_dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
	...
	struct mdio_if_info const* ptr)
{
	...
	{
		if (*dev_ptr) {
			callee_marshal_kernel__mdio45_probe__mdio_if_info_dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
	...
	struct mdio_if_info* ptr)
{
	...
	{
		if (*dev_ptr) {
			caller_unmarshal_kernel__mdio45_probe__mdio_if_info_dev__in(__pos, msg, ext, ctx, *dev_ptr);
		}
