...
int mei_klcd_dispatch_async_loop(struct fipc_message *message);
int mei_klcd_syncipc_dispatch(struct fipc_message *message);

static int mei_klcd_init(void)
{
	int ret;
	 ...
	 */
	ret = glue_mei_init();
	LIBLCD_MSG("-===== > glue mei init called\n");
	if (ret) {
		LIBLCD_ERR("net init");
		goto fail2;
	}
	/* register with vmfunc */
	vmfunc_init(lcd_stack, mei_klcd_dispatch_async_loop, mei_klcd_syncipc_dispatch);
 ...
 */
static void __exit mei_klcd_exit(void)
{
	struct fipc_message m;
...
module_init(mei_klcd_init);
module_exit(mei_klcd_exit);
MODULE_LICENSE("GPL");
