...
static int boot_main(void)
{
	...
	m = lvd_create_module_klcd_no_thread(LCD_DIR("meime_gen/misc_klcd"),
				"lcd_test_mod_meime_gen_misc_klcd",
				&misc_klcd);
...
fail9:
fail8:
	lcd_cap_delete(meime_lcd);
	lcd_destroy_create_ctx(dummy_ctx);
fail4:
	//lcd_cap_delete(misc_klcd);
	lcd_destroy_module_klcd(misc_klcd, "lcd_test_mod_meime_gen_misc_klcd");
fail3:
...
int boot_lcd_thread(void *data)
{
	...
	if (!ret) {
		...
		lcd_destroy_module_klcd(misc_klcd,
				"lcd_test_mod_meime_gen_misc_klcd");
		if (current->lcd)
			lcd_cap_delete(meime_lcd);
		if (dummy_ctx)
