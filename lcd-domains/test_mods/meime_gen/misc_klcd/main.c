...
int misc_klcd_dispatch_loop(struct fipc_message *msg)
{
...
int misc_klcd_syncipc_dispatch(struct fipc_message *message)
{
	glue_user_panic("Shouldn't have done a synchronous call!\n");
	return -1;
}

static int misc_klcd_init(void) 
{
	...
	vmfunc_init(lcd_stack, misc_klcd_dispatch_loop, misc_klcd_syncipc_dispatch);
 ...
 */
static void __exit misc_klcd_exit(void)
{
	struct fipc_message m = {0};
...
module_init(misc_klcd_init);
module_exit(misc_klcd_exit);
MODULE_LICENSE("GPL");
