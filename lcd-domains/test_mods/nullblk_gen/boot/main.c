...
cptr_t blk_klcd, nullblk_lcd;
struct lcd_create_ctx *nullblk_ctx;
...
static int boot_main(void)
{
	...
	m = lvd_create_module_klcd_no_thread(LCD_DIR("nullblk_gen/blk_klcd"),
				"lcd_test_mod_nullblk_gen_blk_klcd",
				&blk_klcd);
...
fail9:
fail8:
	lcd_cap_delete(nullblk_lcd);
	lcd_destroy_create_ctx(nullblk_ctx);
fail4:
	//lcd_cap_delete(blk_klcd);
	lcd_destroy_module_klcd(blk_klcd, "lcd_test_mod_nullblk_gen_blk_klcd");
fail3:
