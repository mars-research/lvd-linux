...
static inline void dump_mci(struct mem_ctl_info *mci, const char *func, int line_no) {
	int i = 0;
	printk("%s:%d -> ", func, line_no);
	for (; i < mci->n_layers; i++) {
		printk("mci->layers[%d].size %u", i, mci->layers[i].size);
	}
}
...
static int get_dimm_config(struct mem_ctl_info *mci)
{
	...
	dump_mci(mci, __func__, __LINE__);
	...
	for (i = 0; i < channels; i++) {
		...
		for (j = 0; j < max_dimms_per_channel; j++) {
			{
				int k = 0;
				printk("%s, mci %p mci->layers %p | mci->n_layers %u", __func__, mci, mci->layers, mci->n_layers);
				for (k = 0; k < 2; k++) {
					printk("layers[%d].size %u", k, mci->layers[k].size);
				}
			}
			printk("%s, i %d, j %d, accessing idx %u\n", __func__, i, j, EDAC_DIMM_OFF(mci->layers, mci->n_layers, i, j, 0));

			dimm = EDAC_DIMM_PTR(mci->layers, mci->dimms, mci->n_layers,
				       i, j, 0);
			if (!dimm) {
				printk("%s, dimm is empty. continuing!\n", __func__);
				continue;
			}
			if (pvt->info.type == KNIGHTS_LANDING) {
			...
			if (IS_DIMM_PRESENT(mtr)) {
				...
				snprintf(dimm->label, sizeof(dimm->label),
					 "CSrcID#%u_Ha#%u_Ch#%u_DIMM#%u",
					 (u8)pvt->sbridge_dev->source_id, (u8)(i/4), (u8)(i%4), (u8)j);
			}
		}
	}
				 ...
				 const int multi_bus)
{
	...
	sbridge_printk(KERN_INFO, "Detected %04x:%04x\n",
		 PCI_VENDOR_ID_INTEL, dev_descr->dev_id);
				 ...
				 struct sbridge_dev *sbridge_dev)
{
	...
	if (saw_chan_mask != 0x0f) {
		printk("%s:%d saw_chan_mask %u\n", __func__, __LINE__, saw_chan_mask);
		goto enodev;
	}
	return 0;

enodev:
	sbridge_printk(KERN_ERR, "%s: Some needed devices are missing\n", __func__);
	return -ENODEV;
				 ...
				 struct sbridge_dev *sbridge_dev)
{
	...
	printk("%s:%d n_devs %d\n", __func__, __LINE__, sbridge_dev->n_devs);
	dump_mci(mci, __func__, __LINE__);
	for (i = 0; i < sbridge_dev->n_devs; i++) {
		pdev = sbridge_dev->pdev[i];
		printk("%s, pdev %p\n", __func__, pdev);
...
static int sbridge_register_mci(struct sbridge_dev *sbridge_dev, enum type type)
{
	...
	printk("%s,tot_dimms %d\n", __func__, mci->tot_dimms); 
