...
void edac_mc_handle_error_callee(struct fipc_message* msg, struct ext_registers* ext)
{
	...
	{
		*_msg_ptr = glue_unpack_new_shadow(__pos, msg, ext, char const*, (sizeof(char) * glue_peek(__pos, msg, ext)), (DEFAULT_GFP_FLAGS));
		if (*_msg_ptr) {
	...
	{
		*other_detail_ptr = glue_unpack_new_shadow(__pos, msg, ext, char const*, (sizeof(char) * glue_peek(__pos, msg, ext)), (DEFAULT_GFP_FLAGS));
		if (*other_detail_ptr) {
...
void edac_mc_free_callee(struct fipc_message* msg, struct ext_registers* ext)
{
	...
	printk("%s:%d calling edac_mc_free with mci %p\n", __func__, __LINE__, mci);
...
void edac_mc_alloc_callee(struct fipc_message* msg, struct ext_registers* ext)
{
	...
	{
		*layers_ptr = glue_unpack_new_shadow(__pos, msg, ext, struct edac_mc_layer*, (sizeof(struct edac_mc_layer) * glue_peek(__pos, msg, ext)), (DEFAULT_GFP_FLAGS));
		if (*layers_ptr) {
			int i;
			...
			for (i = 0; i < len; ++i) {
				...
				callee_unmarshal_kernel__edac_mc_alloc__layers__in(__pos, msg, ext, ctx, element);
			}
	...
	printk("%s, n_layers %u\n", __func__, n_layers);
	{
		...
		for (i = 0; i < n_layers; i++) {
			printk("ty %d | sz %u | isvirt %d\n",
					layers[i].type, layers[i].size, layers[i].is_virt_csrow);
		}
	}
