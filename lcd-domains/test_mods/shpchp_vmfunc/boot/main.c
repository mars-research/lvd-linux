 ...
 *          Launches shpchp lcd and pci klcd modules and waits
...
cptr_t pci_klcd, shpchp_lcd;
struct lcd_create_ctx *shpchp_ctx;
...
static int boot_main(void)
{
	...
	m = lvd_create_module_klcd_no_thread(LCD_DIR("shpchp_vmfunc/pci_klcd"),
				"lcd_test_mod_shpchp_vmfunc_pci_klcd",
				&pci_klcd);
	...
	/* ---------- Create LCD ---------- */
	ret = lvd_create_module_lvd(LCD_DIR("shpchp_vmfunc/shpchp_lcd"),
				"lcd_test_mod_shpchp_vmfunc_shpchp_lcd",
...
fail5:
fail4:
	lcd_cap_delete(shpchp_lcd);
	lcd_destroy_create_ctx(shpchp_ctx);
fail3:
	//lcd_cap_delete(pci_klcd);
	lcd_destroy_module_klcd(pci_klcd, "lcd_test_mod_shpchp_vmfunc_pci_klcd");
fail2:
...
int boot_lcd_thread(void *data)
{
	...
	if (!ret) {
		...
		lcd_destroy_module_klcd(pci_klcd,
				"lcd_test_mod_shpchp_vmfunc_pci_klcd");
