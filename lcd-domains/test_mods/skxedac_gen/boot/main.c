...
cptr_t edac_klcd, skx_lcd;
struct lcd_create_ctx *skx_ctx;
...
static int boot_main(void)
{
	...
	m = lvd_create_module_klcd_no_thread(LCD_DIR("skxedac_gen/edac_klcd"),
				"lcd_test_mod_skxedac_gen_edac_klcd",
				&edac_klcd);
	...
	ret = lvd_create_module_lvd(LCD_DIR("skxedac_gen/skx_lcd"),
				"lcd_test_mod_skxedac_gen_skx_lcd",
				&skx_lcd,
				&skx_ctx, 1);
	if (ret) {
		LIBLCD_ERR("failed to create skx lcd");
		goto fail4;
	}
	...
	LIBLCD_MSG("starting skx EDAC...");

	ret = lcd_run(skx_lcd);
	if (ret) {
		LIBLCD_ERR("failed to start skx lcd");
		goto fail9;
	}
...
fail9:
fail8:
	lcd_cap_delete(skx_lcd);
	lcd_destroy_create_ctx(skx_ctx);
fail4:
	//lcd_cap_delete(edac_klcd);
	lcd_destroy_module_klcd(edac_klcd, "lcd_test_mod_skxedac_gen_edac_klcd");
fail3:
