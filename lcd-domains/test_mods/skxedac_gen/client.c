...
void notifier_call_callee(struct fipc_message* msg, struct ext_registers* ext)
{
	...
	{
		*data_ptr = glue_unpack_new_shadow(__pos, msg, ext, void*, (sizeof(struct mce)));
		if (*data_ptr) {
		}
...
void edac_mc_handle_error(unsigned int type, struct mem_ctl_info* mci, unsigned short error_count, unsigned long page_frame_number, unsigned long offset_in_page, unsigned long syndrome, int top_layer, int mid_layer, int low_layer, char const* _msg, char const* other_detail)
{
	...
	{
		const void* __adjusted = *_msg_ptr;
		glue_pack(__pos, msg, ext, __adjusted);
		if (*_msg_ptr) {
			size_t i, len;
	...
	{
		const void* __adjusted = *other_detail_ptr;
		glue_pack(__pos, msg, ext, __adjusted);
		if (*other_detail_ptr) {
			size_t i, len;
...

	{
		...
		if (*layers_ptr) {
			...
			for (i = 0; i < len; ++i) {
				...
				caller_marshal_kernel__edac_mc_alloc__layers__io(__pos, msg, ext, ctx, element);
			}
	...
	*__pos = 0;
...

	{
		if (*layers_ptr) {
			...
			for (i = 0; i < len; ++i) {
				struct edac_mc_layer* element = &array[i];
				caller_unmarshal_kernel__edac_mc_alloc__layers__io(__pos, msg, ext, ctx, element);
			}
	...
	{
		*ret_ptr = glue_unpack_new_shadow(__pos, msg, ext, struct mem_ctl_info*, (sizeof(struct mem_ctl_info) + ctx->sz_pvt));
		if (*ret_ptr) {
...
struct pci_dev* pci_get_device(unsigned int vendor, unsigned int device, struct pci_dev* from)
{
	...
	if (from)
	{
		const void* __adjusted = *from_ptr;
		glue_pack_shadow(__pos, msg, ext, __adjusted);
		if (*from_ptr) {
