...
int edac_klcd_dispatch_loop(struct fipc_message *msg)
{
...
static int edac_klcd_init(void) 
{
	...
	vmfunc_init(lcd_stack, edac_klcd_dispatch_loop, edac_klcd_syncipc_dispatch);

	/* call module_init for lcd */
	m.vmfunc_id = VMFUNC_RPC_CALL;
	m.rpc_id = MODULE_INIT;
	LIBLCD_MSG("vmfunc_init successfull! Calling MODULE_INIT of skx_lcd");
...
static void __exit edac_klcd_exit(void)
{
	...
	LIBLCD_MSG("Calling MODULE_EXIT of skx_lcd");
	vmfunc_klcd_wrapper(&m, OTHER_DOMAIN);
