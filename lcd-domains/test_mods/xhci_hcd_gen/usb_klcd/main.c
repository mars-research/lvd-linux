/*
 * main.c - module init/exit for usb klcd
 */
...
int usb_klcd_dispatch_loop(struct fipc_message *msg)
{
...
int usb_klcd_syncipc_dispatch(struct fipc_message *message)
{
	glue_user_panic("Shouldn't have done a synchronous call!\n");
	return -1;
}

static int _usb_klcd_init(void)
{
	int ret;
	...
	vmfunc_init(lcd_stack, usb_klcd_dispatch_loop, usb_klcd_syncipc_dispatch);
 ...
 */
static void __exit _usb_klcd_exit(void)
{
	struct fipc_message m = {0};
...
module_init(_usb_klcd_init);
module_exit(_usb_klcd_exit);
MODULE_LICENSE("GPL");
